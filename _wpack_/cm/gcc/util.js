        ...
        if (headerId == "chk") {
          ...
          for (let i = 0; i < rowCount; i++) {
            ...
            if (newValue) {
              ...
              dltObj.modifyRowStatus(realRowIndex, "R");
            }
          }
          if (newValue) {
...
scwin._setGridViewRowCheckBox = function ($p, gridViewObj, rowIndex, newValue) {
  ...
  let rowIndexArr = "";
  if (newValue === false) {
    dltObj.modifyRowStatus(rowIndex, "R");
  } else {
    rowIndexArr = gridViewObj.getCheckedIndex("chk");
...
/**
 * @method 그리드 시간 validate
 * @name gridTimeValidate
 * @description desc
 * @param {string} v desc
 * @hidden N
 * @exception 
 * @example ${example}
 */
scwin.gridTimeValidate = function ($p, v) {
  v = (v ?? "").toString().trim();
  if (!v) return; // 빈값 허용이면 OK
