/*amd /cm/gcc/hkey.xml 24406 32bba2479ee58352ec80de263d5c45439dba36aa84999df2d50390a2820fbe5c */
  ...
  try {
    ...
    if (lastKey != "Control" && lastKey != "Alt" && lastKey != "Shift") {
      ...
      if (scwin.__isPreventKey(complexKey, lastKey)) {
        scwin.__runEvent(complexKey, lastKey, e);
 ...
 * @example
 */
scwin.__runEvent = function (complexKey, eventKey, e) {
  try {
    ...
    if (typeof $p.top().tac_layout === "undefined") {
      // 팝업
      activeWindowInfo = [];
      activeWindowInfo["window"] = WebSquare.getBody().getScopeWindow();
    } else {
      activeWindowInfo = $c.win.getActiveWindowInfo($p, scopeApi);
    }
    // const activeWindowInfo = [];
    let findframe = activeWindowInfo["window"]; // 단축키가 감지된 프레임

    if (eventKey == "F3") {
      if (typeof findframe.btn_Save !== "undefined") {
        findframe.btn_Save.trigger("onclick");
      } else if (typeof findframe.btn_save !== "undefined") {
        findframe.btn_save.trigger("onclick");
      } else {
        let tabObj = Object.values(findframe);
        for (let idx in tabObj) {
          // tabControl 일 경우 
          if (typeof tabObj[idx].getPluginName !== "undefined" && tabObj[idx].getPluginName() == "tabControl") {
            // debugger;
            let curIdx = tabObj[idx].getSelectedTabIndex(); // 현재 선택된 탭index
            if (typeof tabObj[idx].getWindow(curIdx).btn_save !== "undefined") {
              tabObj[idx].getWindow(curIdx).btn_save.trigger("onclick");
              break;
            } else if (typeof tabObj[idx].getWindow(curIdx).btn_Save !== "undefined") {
              tabObj[idx].getWindow(curIdx).btn_Save.trigger("onclick");
              break;
            }
          }
    ...
    if (typeof searchEvent != "undefined" && searchEvent.length > 0) {
      ...
      const funcTokenArr = shortuctObj["EVENT_NAME"];
      if (funcTokenArr) {
        let fnObj = funcTokenArr.split(".");
        let fn = findframe[fnObj[0]][fnObj[1]];
        fn();
      }
    }
...
scwin.__isPreventKey = function (complexKey, lastKey) {
  ...
  if (exTag.indexOf(activeTag) === -1) {
    ...
    if (complexKey === "singleKey" && lastKey === "Tab" || complexKey === "shiftKey" && lastKey === "Tab") {
      return false;
    } else {
      return true;
    }
