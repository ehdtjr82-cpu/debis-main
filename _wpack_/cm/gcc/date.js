...
scwin.isDate = function ($p, sDate, timeChk) {
  ...
  try {
    if (sDate.length == 4) {
      sDate = sDate + "0101000000";
    } else if (sDate.length == 6) {
      sDate = sDate + "01000000";
...
/**
 * @method
 * @name getDateAddMonthDirection
 * @description n개월 이전/이후 마지막 일자 
 * @param {String} v 날짜 형식의 문자열 (yyyyMMdd or yyyyMMddHHmmss)
 * @param {Number} offset 더할 분 수
 * @param {String} direction 월의 첫일, 마지막일
 * @returns 첫일, 마지막일의 날짜
 * @hidden N
 * @exception 
 * @example
$c.date.getDateAddMonthDirection("20251106", -1 , "L");   //20251031
 */
scwin.getDateAddMonthDirection = function ($p, v, offset, direction) {
  try {
    let date;
    switch (direction) {
      case "F":
        date = $c.date.addMonth($p, v, offset).substring(0, 6) + "01";
        break;
      case "L":
        date = $c.date.addMonth($p, v, offset);
        date = date.substring(0, 6) + $c.date.getLastDateOfMonth($p, date);
        break;
    }
    return date;
  } catch (ex) {
...
/**
 * @method
 * @name getDateQuarterDirection
 * @description n개월 분기 이전/이후 마지막 일자 
 * @param {String} v 날짜 형식의 문자열 (yyyyMMdd or yyyyMMddHHmmss)
 * @param {String} direction 월의 첫일, 마지막일
 * @returns 첫일, 마지막일의 날짜
 * @hidden N
 * @exception 
 * @example
$c.date.getDateQuarterDirection("20251106", "L");   //20251231
 */
scwin.getDateQuarterDirection = function ($p, v, direction) {
  try {
    let assemble;
    ...
    switch (month) {
      ...
      case "03":
        if (direction === "F") {
          ...
          assemble = "03" + $c.date.getLastDateOfMonth($p, v.substring(0, 4) + "03");
        }
        break;
      case "04":
      case "05":
      case "06":
        if (direction === "F") {
          ...
          assemble = "06" + $c.date.getLastDateOfMonth($p, v.substring(0, 4) + "06");
        }
        break;
      case "07":
      case "08":
      case "09":
        if (direction === "F") {
          ...
          assemble = "09" + $c.date.getLastDateOfMonth($p, v.substring(0, 4) + "09");
        }
        break;
      case "10":
      case "11":
      case "12":
        if (direction === "F") {
          ...
          assemble = "12" + $c.date.getLastDateOfMonth($p, v.substring(0, 4) + "12");
        }
        break;
    }
    return v.substring(0, 4) + assemble;
  } catch (ex) {
    console.error(ex);
    return null;
  }
 ...
 * @method
 * @name diffDateMsg
 * @description 시작일이 클경우 메시지 호출
 * @param {string} date1 시작일
 * @param {string} date2 종료일
 * @returns
 * @hidden N
 * @exception
 * @example
 */
scwin.diffDateMsg = async function ($p, date1, date2) {
  if ($c.date.diffDate($p, date1, date2) < 0) {
    await $c.win.alert($p, "시작일자가 종료일자 이전이여야 합니다.");
    return false;
  }
  ;
  return true;
 ...
 * @example
 */
scwin.__validDate = function (val) {
  let ret = val;
  if (!$c.date.isDate($p, val) && val != "") {
 ...
 * @method
 * @name __validDate2
 * @description 월에 00 입력 가능 validator
 * @param {string} val desc
 * @returns
 * @hidden N
 * @exception
 * @example
 */
scwin.__validDate2 = function (val) {
  let ret = val;
