 ...
 * @exception 
 */
scwin.__callbackSubmitFunction = async function (resObj, sbmObj) {
  const $p = $c.win.__getScope(sbmObj).scwin.$w;
  if (typeof resObj.responseJSON !== "undefined" && typeof resObj.responseJSON.resultDataSet !== "undefined" && resObj.responseJSON.resultDataSet[0].Code == -1 && sbmObj.userData1 == "noError") {
    var msgObj1 = {
      statusCode: "E",
      errorCode: resObj.responseJSON.resultDataSet[1].Code,
      message: resObj.responseJSON.resultDataSet[1].Msg
    };
    let msg = await $c.sbm.resultMsg($p, msgObj1, true);
    resObj.responseJSON.resultDataSet[1].Msg = msg;
  } else if (typeof resObj.responseJSON !== "undefined" && typeof resObj.responseJSON.resultDataSet !== "undefined" && resObj.responseJSON.resultDataSet[0].Code == -1 && sbmObj.userData1 != "noError") {
    var msgObj1 = {
      statusCode: "E",
      errorCode: resObj.responseJSON.resultDataSet[1].Code,
      message: resObj.responseJSON.resultDataSet[1].Msg
...
scwin.__submitErrorHandler = function (resObj) {
  ...
  let msgObj = "";
  if (resObj.responseStatusCode == 504) {
    msgObj = {
      statusCode: "E",
      errorCode: "E504",
      message: "연결 시간을 초과하였습니다.",
      messageDetail: detailStr
    };
  } else {
    msgObj = {
...
scwin.shouldSuppressMissingLoginAlert = function ($p) {
  const href = (location.href || "").toLowerCase();
  if (href.indexOf("ebiz_index.xml") > -1 || href.indexOf("externalsabunloginpopup") > -1) {
    return true;
  }
  try {
    const until = parseInt(window.localStorage.getItem("ebizSuppressMissingLoginUntil") || "0", 10);
    if (until > Date.now()) {
      return true;
    }
    if (until > 0) {
      window.localStorage.removeItem("ebizSuppressMissingLoginUntil");
    }
  } catch (e) {}
  return false;
};
 ...
 * @exception 
 */
scwin.resultMsg = async function ($p, resultData, types) {
  ...
  switch (resultData.statusCode) {
    case msgCode.STATUS_ERROR:
      if (resultData.errorCode == "E0001") {
        const alertMsg = $c.data.getMessage($p, MSG_CM_00003);
        $c.win.alert($p, alertMsg, function () {
          $c.win.reload($p);
        });
      } else if (resultData.errorCode == "4") {
        ...
        // 3. 메시지코드 (8자리)
        const msgCode = parts[0].substring(0, 8);
        ...
        // 6. 개행 처리
        const msgMainMultiLine = msgMain.replace(/<br>/g, "\n");
        const msgMesursItmMultiLine = msgMesursItm.replace(/<br>/g, "\n");
        ...
        if (msgMesursItm) {
          resultMsg += "조 치 사 항: " + msgMesursItmMultiLine;
        }
        // debugger;
        if (typeof types !== "undefined" && types) {
          return resultMsg;
        } else {
          $c.win.alert($p, resultMsg);
        }
      } else if ($c.util.isEmpty($p, message) === false) {
        if (message == "로그인 정보가 없습니다. 다시 로그인 하십시오.") {
          if (scwin.shouldSuppressMissingLoginAlert($p)) {
            return;
          }
          await $c.win.alert($p, message);
          // 로그아웃 상태일 경우 로그인 팝업 호출 
          $c.win.openloginPopup($p);
        } else {
 ...
 * @example  
 */
scwin.trAbort = function ($p, opts) {
  try {
    opts = opts || {};
    trID = opts.trID, trFlag = opts.trFlag ? opts.trFlag : false, delayTime = opts.delayTime;
    let subID = $c.gus.object($p, trID);
    if (subID && trFlag && delayTime) {
      $p.setTimeout(function () {
        WebSquare.ModelUtil.abort(subID.id);
      }, {
        delay: delayTime
      });
    }
  } catch (e) {
    console.log("scwin.trAbort=>" + e);
  }
};
