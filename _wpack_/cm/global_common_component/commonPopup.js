/*amd /cm/global_common_component/commonPopup.xml 8898 4227239a4d667b18f45ae9d3130da6fb88c728068cfd57bffb034d133dc30ed0 */
...
scwin._openPopup = function (_$p, url, opt, data) {
  ...
  if (opt.type == "browserPopup") {
    var top = Math.floor((window.screen.availHeight - 50 - $c.num.parseInt($p, height)) / 2) + (window.screen.availTop || 0) + "px";
    var left = Math.floor((window.screen.availWidth - com.num.parseInt(width)) / 2) + (window.screen.availLeft || 0) + "px";
  } else {
    var top = document.body.offsetHeight / 2 - $c.num.parseInt($p, height) / 2 + $(document).scrollTop() + "px";
    var left = document.body.offsetWidth / 2 - $c.num.parseInt($p, width) / 2 + $(document).scrollLeft() + "px";
  }
  debugger;
