/*amd /cm/global_common_component/commonStr.xml 15214 586545f15a0bcdb8d63f8f3b0290635e5dd68c3586c864b903512ee0758c0b72 */
define({declaration:{A:{version:'1.0',encoding:'UTF-8'}},E:[{T:1,N:'html',A:{xmlns:'http://www.w3.org/1999/xhtml','xmlns:ev':'http://www.w3.org/2001/xml-events','xmlns:w2':'http://www.inswave.com/websquare','xmlns:xf':'http://www.w3.org/2002/xforms'},E:[{T:1,N:'head',A:{},E:[{T:1,N:'w2:type',E:[{T:3,text:'COMPONENT'}]},{T:1,N:'w2:buildDate'},{T:1,N:'w2:MSA'},{T:1,N:'xf:model',E:[{T:1,N:'w2:dataCollection',A:{baseNode:'map'}},{T:1,N:'w2:workflowCollection'}]},{T:1,N:'w2:layoutInfo'},{T:1,N:'w2:publicInfo',A:{method:'scwin.lpad,scwin.rpad,scwin.formatSSN,scwin.formatPhone,                                scwin.formatTime,scwin.getLocale,scwin.existKorean,scwin.isKorean,                                scwin.isFinalConsonant,scwin.attachPostposition,scwin.isBizID,scwin.isSSN,                                scwin.isEmail,scwin.isPhone,scwin.replaceAll,scwin.trim,scwin.getByteLength,scwin.formatTime,scwin.isFinalConsonant,scwin.isEmail'}},{T:1,N:'script',A:{lazy:'false',type:'text/javascript'},E:[{T:4,cdata:function(scopeObj){with(scopeObj){/**
 * @method
 * @name lpad
...
scwin.rpad = function (_$p, str, length, charVal) {
  ...
  while (str.length < length) str += charVal;
  str = str.length >= length ? str.substring(0, length) : str;
  return str;
};
...
scwin.attachPostposition = function (_$p, str) {
  if ($c.util.getLanguage($p, _$p, "ko") === true) {
...
scwin.isSSN = function (_$p, str) {
  ...
  if (parseInt($c.util.getServerDateTime($p, _$p, "yyyy")) - parseInt(yy + str.substring(0, 2)) < 0) {
    return false;
  }

  // 외국인 주민번호 체크로직 추가
  if (str.charAt(6) != '5' && str.charAt(6) != '6' && str.charAt(6) != '7' && str.charAt(6) != '8') {
    if (temp == $c.num.parseInt($p, _$p, str.charAt(12))) {
      return true;
    } else {
      return false;
    }
  } else {
    if ((temp + 2) % 10 == $c.num.parseInt($p, _$p, str.charAt(12))) {
      return true;
    } else {
      return false;
    }
