...
scwin.setCommonCode = function (_$p, codeOptions, callbackFunc) {
  ...
  for (i = 0; i < codeOptionsLen; i++) {
    ...
    try {
      ...
      if (typeof scwin.commonCodeList[dltId] === "undefined") {
        ...
        dataListObj.setJSON($c.util.getJSON($p, _$p, scwin.commonCodeList[dltId]));
      }
      if (codeObj.compID) {
  ...
  if (paramCode !== "") {
    ...
    var sbmObj = $c.util.getComponent($p, _$p, searchCodeGrpOption.id);
    var reqData = {
      "dma_commonCode": {
        "GRP_CD": paramCode,
