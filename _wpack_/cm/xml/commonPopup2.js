/*amd /cm/xml/commonPopup2.xml 17169 1341609aabb1280cc497c93e88d1bd2c7c2f4733b5704fcac85470fc48a355fe */
...
scwin.f_ByteLengthChk = function (str1, str2) {
  ...
  if (parseInt(scwin.nLenCD) && len1 > 0) {
    if (len1 < scwin.nLenCD) {
      alert("검색어를 " + scwin.nLenCD + "자리 이상 입력 하세요");
      ed_code.focus();
      return false;
    }
  }
  if (parseInt(scwin.nLenNM) && len2 > 0) {
    if (len2 < scwin.nLenNM) {
      alert("검색어를 " + scwin.nLenNM + "자리 이상 입력 하세요");
      ed_name.focus();
      return false;
    }
  }
...
scwin.f_Retrieve = async function (arg_ds, init_gb) {
  ...
  let lux_value2 = "";
  if (ed_year.getValue() == "") {
    await $c.win.alert($p, "회계연도를 입력하세요. ");
    ed_year.focus();
    return;
  }
