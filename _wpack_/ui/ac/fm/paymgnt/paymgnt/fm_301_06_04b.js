...
scwin.f_Save = async function () {
  try {
    ...
    */
    ...
    if (await $c.win.confirm($p, $c.data.getMessage($p, MSG_CM_CRM_009, "전자결제파일생성-국민은행"))) {
      ds_cont.removeAll();
      sbm_save.action = "/ac.fm.paymgnt.paymgnt.CreateKbbFileCMD.do";
...
scwin.sbm_save_submitdone = function (e) {
  try {
    ...
    if (ds_cont.getRowCount() > 0) {
      for (let i = 0; i < ds_cont.getRowCount(); i++) {
        ds_cont.setCellData(i, "cont1", ds_cont.getCellData(i, "cont").substring(0, 10));
        ds_cont.setCellData(i, "cont2", ds_cont.getCellData(i, "cont").substring(10, 20));
        ds_cont.setCellData(i, "cont3", ds_cont.getCellData(i, "cont").substring(20, 28));
        ds_cont.setCellData(i, "cont4", ds_cont.getCellData(i, "cont").substring(28, 41));
      }
      scwin.f_createFile();
      $c.gus.cfAlertMsg($p, $c.data.getMessage($p, MSG_CM_INF_012, "전자결제파일생성-국민은행"));
    } else {
