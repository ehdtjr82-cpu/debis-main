...
scwin.onpageload = function () {
  ...
  console.log(scwin.params.arrParm);
...
scwin.initPop = function (obj) {
  ...
  ed_year.setValue(arrParent[1][2].trim()); // 넘머온 Year Value SET
...
scwin.f_Retrieve = async function (arg_ds, init_gb) {
  ...
  if (init_gb == "0") {
    //await $c.win.alert(arg_ds.ExportData(1,arg_ds.CountRow, true));
    ...
    //await $c.win.alert(ds_popup.ExportData(1,ds_popup.CountRow, true));
  } else {
    ds_comcode.set("sysCd", "CommonEBC");
    ds_comcode.set("queryId", scwin.strQueryId);
    ds_comcode.set("userId", $c.data.getMemInfo($p, "userId"));
    ds_comcode.set("param1", ed_code.getValue()); //귀속부서코드
    ds_comcode.set("param2", ed_name.getValue()); //귀속부서명
    ds_comcode.set("param3", ed_year.getValue()); //회계연도
