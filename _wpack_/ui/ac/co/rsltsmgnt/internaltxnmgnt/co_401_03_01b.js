 ...
 * 테스트데이터 
 *    1. 회사코드[000], 거래일자[20100101~20101231], 매입부서[00009], 진행상태[전채]
 * 수정이력 :
 *    - 2026-01-08      배기원    최초작성
 */
//hidden 값
scwin.tempUseYn = ""; //asis 사용안함
scwin.wrkNo = ""; //작업구분코드
scwin.fromDate = ""; //년월
...
scwin.f_FieldClear = function () {
  ...
  //ed_deptCd.value = "<%=login.getAcctDeptNm()%>";
  //부서값 있어서 주석
...
scwin.f_Pchs = async function () {
  ...
  //         await $c.gus.cfAlertMsg("청구상태인 data만 매입처리할 수 있습니다.");
  //         return;
  //     }
  //     //마감여부 확인
  //     if (ds_txn.getCellData(i, "selYn") == "1") {
  ...
  for (let i = 0; i < row.length; i++) {
    if (ds_txn.getCellData(row[i], "internalTxnStsCd") != ACConstants.INTERNAL_TXN_BILG) {
      await $c.gus.cfAlertMsg($p, "청구상태인 data만 매입처리할 수 있습니다.");
      return;
    }
  ...
  if (ds_txn.getModifiedIndex().length < 1) {
    await $c.gus.cfAlertMsg($p, "매입처리할 data가 없습니다.");
    return;
  }

  //청구할 것인지 confirm
  if (await $c.win.confirm($p, "매입처리하시겠습니까?")) {
...
scwin.f_CancelPchs = async function () {
  ...
  //     if (ds_txn.getCellData(i, "selYn") == "1" &&
  //         ds_txn.getCellData(i, "internalTxnStsCd") != ACConstants.INTERNAL_TXN_BILG) {
  //         await $c.gus.cfAlertMsg("청구상태인 data만 매입거부할 수 있습니다.");
  //         return;
  //     }
  //     //마감여부 확인
  //     if (ds_txn.getCellData(i, "selYn") == "1") {
  ...
  for (let i = 0; i < row.length; i++) {
    if (ds_txn.getCellData(row[i], "internalTxnStsCd") != ACConstants.INTERNAL_TXN_BILG) {
      await $c.gus.cfAlertMsg($p, "청구상태인 data만 매입거부할 수 있습니다.");
      return;
    }
    //마감여부 확인
    let txnDt = ds_txn.getCellData(row[i], "txnDt");
  ...
  // 변경된 사항이 있는지 확인.
  if (ds_txn.getModifiedIndex().length < 1) {
    await $c.gus.cfAlertMsg($p, "매입거부할 data가 없습니다.");
    return;
  }
...
scwin.f_CancelPchsReject = async function () {
  ...
  // for (let i=0; i<row; i++) {
  //     if (ds_txn.getCellData(i, "selYn") == "1" &&
  //         ds_txn.getCellData(i, "internalTxnStsCd") != ACConstants.INTERNAL_TXN_DCSN_CNCL) {
  //         await $c.gus.cfAlertMsg("확정취소인 data만 매입거부취소 수 있습니다.");
  //         return;
  //     }
  //     //마감여부 확인
  //     if (ds_txn.getCellData(i, "selYn") == "1") {
  ...
  var row = ds_txn.getMatchedIndex("selYn", "1");
  for (let i = 0; i < row.length; i++) {
    if (ds_txn.getCellData(row[i], "internalTxnStsCd") != ACConstants.INTERNAL_TXN_DCSN_CNCL) {
      await $c.gus.cfAlertMsg($p, "확정취소인 data만 매입거부취소 수 있습니다.");
      return;
    }
    //마감여부 확인
    let txnDt = ds_txn.getCellData(row[i], "txnDt");
  ...
  // 변경된 사항이 있는지 확인.
  if (ds_txn.getModifiedIndex().length < 1) {
    await $c.gus.cfAlertMsg($p, "매입거부 취소할 data가 없습니다.");
    return;
  }

  //청구취소 할 것인지 confirm
  if (await $c.win.confirm($p, "매입거부 취소 하시겠습니까?")) {
    sbm_cancelTxnReject.userData1 = "noError";
    $c.sbm.execute($p, sbm_cancelTxnReject);
  }
};
scwin.sbm_cancelTxnReject_submitdone = async function (e) {
  if (e.responseJSON.resultDataSet[0].Code < 0) {
    await $c.gus.cfAlertMsg($p, e.responseJSON.resultDataSet[1].Msg);
    return;
  }
...
scwin.sbm_cancelTxnReject_submiterror = function (e) {
  $c.gus.cfAlertMsg($p, e.responseJSON.resultDataSet[1].Msg);
};
...
scwin.f_Bill = async function () {
  ...
  // for (let i=0; i<row; i++) {
  //     if (ds_txn.getCellData(i, "selYn") == "1" &&
  //         ds_txn.getCellData(i, "internalTxnStsCd") != ACConstants.INTERNAL_TXN_DCSN) {
  //         await $c.gus.cfAlertMsg("확정상태인 data만 매입취소할 수 있습니다.");
  //         return;
  //     }
  //     //마감여부 확인
  //     if (ds_txn.getCellData(i, "selYn") == "1") {
  ...
  var row = ds_txn.getMatchedIndex("selYn", "1");
  for (let i = 0; i < row.length; i++) {
    if (ds_txn.getCellData(row[i], "internalTxnStsCd") != ACConstants.INTERNAL_TXN_DCSN) {
      await $c.gus.cfAlertMsg($p, "확정상태인 data만 매입취소할 수 있습니다.");
      return;
    }
    let txnDt = ds_txn.getCellData(row[i], "txnDt");
    ret = await scwin.f_JobCloseYN($c.gus.cfIsNull($p, txnDt) ? "" : txnDt.substring(0, 6));
  ...
  // 변경된 사항이 있는지 확인.
  if (ds_txn.getModifiedIndex().length < 1) {
    await $c.gus.cfAlertMsg($p, "매입취소할 data가 없습니다.");
    return;
  }

  //청구할 것인지 confirm
  if (await $c.win.confirm($p, "매입취소하시겠습니까?")) {
    sbm_billTxn2.userData1 = "noError";
    $c.sbm.execute($p, sbm_billTxn2);
  }
};
scwin.sbm_billTxn2_submitdone = async function (e) {
  if (e.responseJSON.resultDataSet[0].Code < 0) {
    await $c.gus.cfAlertMsg($p, e.responseJSON.resultDataSet[1].Msg);
    return;
  }
...
scwin.sbm_billTxn2_submiterror = function (e) {
  $c.gus.cfAlertMsg($p, e.responseJSON.resultDataSet[1].Msg);
};
 ...
 * 매입부서팝업
 */
scwin.f_openPushDeptPopUp = function (check) {
  //asis 소스
...
scwin.udc_deptBilgCd_callBackFunc = function (ret) {
  if (ret[0] != "N/A" && !$c.gus.cfIsNull($p, ret)) {
    ed_deptBilgCd.setValue(ret[0]); // 매출부서코드
    ed_deptBilgNm.setValue(ret[1]); // 매출부서명
    ed_deptBilgCd.hidVal = ret[0]; // 히든값
    dma_search.set("aggrBilgDeptYn", ret[8]); // 집계부서여부
  } else {
    ed_deptBilgCd.setValue(""); // 매출부서코드
    ed_deptBilgNm.setValue(""); // 매출부서명
    ed_deptBilgCd.hidVal = ""; // 히든값
    dma_search.set("aggrBilgDeptYn", ""); // 집계부서여부
  }
};
...
scwin.f_GridToExcel = function (gr_groupCode, gridName) {
  ...
  //     $c.win.alert("내부거래매입내역이 존재하지않습니다.");
  //     return false
  // }
...
scwin.udc_deptCd_onclickEvent = function (e) {
  ...
  scwin.f_openPushDeptPopUp(close);
};
...
}}}]},{T:1,N:'w2:require',A:{as:'udc_topGrdBtn',type:'page',variableClone:'true',src:'/cm/udc/topGrdBtn.xml'}},{T:1,N:'w2:require',A:{as:'udc_comCode',type:'page',variableClone:'true',src:'/cm/udc/comCode.xml'}},{T:1,N:'w2:require',A:{as:'udc_fromToCalendar',type:'page',variableClone:'true',src:'/cm/udc/fromToCalendar.xml'}},{T:1,N:'w2:require',A:{as:'udc_comChkRes',type:'page',variableClone:'true',src:'/cm/udc/comChkRes.xml'}}]},{T:1,N:'body',A:{'ev:onpageload':'scwin.onpageload'},E:[{T:1,N:'xf:group',A:{class:'sub_contents',id:'',style:''},E:[{T:1,N:'w2:wframe',A:{id:'',src:'/cm/xml/contentHeader.xml',style:''}},{T:1,N:'xf:group',A:{class:'shbox',id:'',style:''},E:[{T:1,N:'xf:group',A:{class:'shbox-inner',id:'',style:''},E:[{T:1,N:'xf:group',A:{class:'w2tb',id:'tb_bilg',style:'',tagname:'table'},E:[{T:1,N:'w2:attributes',E:[{T:1,N:'w2:summary'}]},{T:1,N:'xf:group',A:{tagname:'colgroup'},E:[{T:1,N:'xf:group',A:{style:'',tagname:'col'}},{T:1,N:'xf:group',A:{style:'',tagname:'col'}},{T:1,N:'xf:group',A:{style:'',tagname:'col'}},{T:1,N:'xf:group',A:{style:'',tagname:'col'}},{T:1,N:'xf:group',A:{style:'',tagname:'col'}},{T:1,N:'xf:group',A:{style:'',tagname:'col'}}]},{T:1,N:'xf:group',A:{class:'',id:'',style:'',tagname:'tr'},E:[{T:1,N:'xf:group',A:{class:'w2tb_th ',style:'',tagname:'th'},E:[{T:1,N:'w2:textbox',A:{class:'req',id:'',label:'회사코드',style:''}}]},{T:1,N:'xf:group',A:{class:'w2tb_td',style:'',tagname:'td'},E:[{T:1,N:'w2:udc_comCode',A:{allowCharCode:'0-9',btnId:'btn_company',code:'coCd',codeId:'ed_coCd',editTypeCode:'focus','ev:onclickEvent':'scwin.udc_comCode_onclickEvent','ev:onviewchangeCodeEvent':'scwin.udc_comCode_onviewchangeCodeEvent',id:'udc_comCode',mandatoryCode:'true',maxlengthCode:'3',maxlengthName:'30',name:'coNm',nameId:'ed_coNm',objTypeCode:'data',objTypeName:'key',popupID:'popupCocd',refDataCollection:'dma_search',selectID:'retrieveDongbuGroupCompanyInfo',style:'',validExpCode:'회사코드',validTitle:'회사코드',codeWidth:'50'}}]},{T:1,N:'xf:group',A:{class:'w2tb_th',style:'',tagname:'th'},E:[{T:1,N:'w2:textbox',A:{class:'req',id:'',label:'거래일자 ',style:''}}]},{T:1,N:'xf:group',A:{class:'w2tb_td',id:'',style:'',tagname:'td'},E:[{T:1,N:'w2:udc_fromToCalendar',A:{edFromId:'ica_inqStDt',edToId:'ica_inqEndDt',id:'udc_fromToCalendar1',mandatoryFrom:'true',mandatoryTo:'true',refDataMap:'dma_search',refEdDt:'txnEndDt',refStDt:'txnStDt',style:''}}]},{T:1,N:'xf:group',A:{class:'w2tb_th',id:'',tagname:'th'},E:[{T:1,N:'w2:attributes'},{T:1,N:'w2:textbox',A:{class:'req',id:'',label:'매입부서',style:''}}]},{T:1,N:'xf:group',A:{class:'w2tb_td',id:'',tagname:'td'},E:[{T:1,N:'w2:attributes'},{T:1,N:'w2:udc_comCode',A:{allowCharCode:'0-9',btnId:'btn_deptCd',code:'acctDeptCd',codeId:'ed_deptCd',editTypeCode:'focus','ev:onclickEvent':'scwin.udc_deptCd_onclickEvent','ev:onviewchangeCodeEvent':'scwin.udc_deptCd_onviewchangeCodeEvent',id:'udc_deptCd',mandatoryCode:'true',maxlengthCode:'5',maxlengthName:'30',name:'acctDeptNm',nameId:'ed_deptNm',objTypeCode:'data',objTypeName:'key',popupID:'popup_deptCd',refDataCollection:'dma_search',selectID:'retrieveAcctDeptCdInfo',style:'',validExpCode:'매입부서',validTitle:'매입부서'}}]}]},{T:1,N:'xf:group',A:{class:'',id:'',style:'',tagname:'tr'},E:[{T:1,N:'xf:group',A:{class:'w2tb_th',style:'',tagname:'th'},E:[{T:1,N:'w2:textbox',A:{class:'',id:'',label:'매출부서',style:''}}]},{T:1,N:'xf:group',A:{class:'w2tb_td',style:'',tagname:'td'},E:[{T:1,N:'w2:udc_comCode',A:{allowCharCode:'0-9',btnId:'btn_deptBilgCd',code:'acctDeptBilgCd',codeId:'ed_deptBilgCd',editTypeCode:'select','ev:onclickEvent':'scwin.udc_deptBilgCd_onclickEvent','ev:onviewchangeCodeEvent':'scwin.udc_deptBilgCd_onviewchangeCodeEvent',id:'udc_deptBilgCd',maxlengthCode:'5',maxlengthName:'30',name:'acctDeptBilgNm',nameId:'ed_deptBilgNm',objTypeCode:'data',objTypeName:'key',popupID:'popup_deptBilgCd',refDataCollection:'dma_search',selectID:'retrieveAcctDeptCdInfo',style:'',validExpCode:'매출부서',validTitle:'매출부서'}}]},{T:1,N:'xf:group',A:{class:'w2tb_th',style:'',tagname:'th'},E:[{T:1,N:'w2:textbox',A:{class:'',id:'',label:'진행상태',style:''}}]},{T:1,N:'xf:group',A:{class:'w2tb_td',id:'',style:'',tagname:'td'},E:[{T:1,N:'w2:autoComplete',A:{class:'form-control w150',editType:'select',id:'lc_internalTxnStsCd',search:'start',style:'',submenuSize:'auto',objType:'data',allOption:'',chooseOption:'true',chooseOptionLabel:'전체',ref:'data:dma_search.internalTxnStsCd',visibleRowNum:'10'},E:[{T:1,N:'w2:choices'}]}]},{T:1,N:'xf:group',A:{class:'w2tb_th',id:'',tagname:'th'},E:[{T:1,N:'w2:attributes'}]},{T:1,N:'xf:group',A:{class:'w2tb_td',id:'',tagname:'td'},E:[{T:1,N:'w2:attributes'}]}]}]}]},{T:1,N:'xf:group',A:{class:'btn-shbox',id:'',style:''},E:[{T:1,N:'xf:trigger',A:{class:'btn ico refresh','ev:onclick':'scwin.f_FieldClear',id:'btn_fieldClear',style:'',type:'button'},E:[{T:1,N:'xf:label',E:[{T:4,cdata:'새로고침'}]}]},{T:1,N:'xf:trigger',A:{class:'btn sch',disabled:'','ev:onclick':'scwin.f_Retrieve',id:'btn_search',style:'',type:'button',userAuth:'R'},E:[{T:1,N:'xf:label',E:[{T:4,cdata:'조회'}]}]}]}]},{T:1,N:'xf:group',A:{class:'round-box',id:'',style:''},E:[{T:1,N:'xf:group',A:{class:'title-wrap',id:'',style:''},E:[{T:1,N:'xf:group',A:{class:'title',id:''},E:[{T:1,N:'w2:textbox',A:{class:'tit',id:'',label:'내부매입현황',style:'',tagname:'h3'}}]},{T:1,N:'xf:group',A:{class:'right',id:''},E:[{T:1,N:'w2:udc_topGrdBtn',A:{grdDownOpts:'{"fileName":"내부거래매입확정.xlsx", "sheetName":"내부거래매입확정", "type":"1", "hiddenVisible":false}',gridID:'gr_txn',gridUpYn:'N',grp:'grd_section1',id:'udc_excel1',style:'',templateYn:'N',gridDownUserAuth:'X'}}]}]},{T:1,N:'xf:group',A:{adaptiveThreshold:'',class:'gvwbox',id:'grd_section1',style:''},E:[{T:1,N:'w2:gridView',A:{autoFit:'none',checkReadOnlyOnPasteEnable:'',class:'wq_gvw',dataList:'data:ds_txn',focusMode:'cell',id:'gr_txn',scrollByColumn:'false',scrollByColumnAdaptive:'false',style:'',visibleRowNum:'14',visibleRowNumFix:'true',readOnly:'true',rowStatusWidth:'20',rowStatusVisible:'true','ev:oncellclick':'scwin.gr_txn_oncellclick',gridName:'내부매입현황',keepDefaultColumnWidth:'true',fixedColumn:'0',fixedColumnNoMove:'true'},E:[{T:1,N:'w2:caption',A:{id:'caption10',style:'',value:'this is a grid caption.'}},{T:1,N:'w2:header',A:{id:'header2',style:''},E:[{T:1,N:'w2:row',A:{id:'row3',style:''},E:[{T:1,N:'w2:column',A:{blockSelect:'false',displayMode:'label',id:'column12',inputType:'checkbox',removeBorderStyle:'false',width:'50',value:' ',fixColumnWidth:'true'}},{T:1,N:'w2:column',A:{blockSelect:'false',displayMode:'label',id:'column11',inputType:'text',removeBorderStyle:'false',width:'110',value:'청구번호'}},{T:1,N:'w2:column',A:{blockSelect:'false',displayMode:'label',id:'column10',inputType:'text',removeBorderStyle:'false',width:'110',value:'시스템'}},{T:1,N:'w2:column',A:{blockSelect:'false',displayMode:'label',id:'column9',inputType:'text',removeBorderStyle:'false',width:'120',value:'내부매입부서'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'column21',inputType:'text',value:'내부매입부서',width:'160'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'column19',inputType:'text',value:'내부매출부서',width:'120'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'column17',inputType:'text',value:'내부매출부서',width:'160'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'column13',inputType:'text',value:'작성일자',width:'100'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'column15',inputType:'text',value:'거래일자',width:'100'}},{T:1,N:'w2:column',A:{width:'100',inputType:'text',id:'column50',value:'실거래일자',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column47',value:'청구처번호',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',style:'',id:'column53',value:'청구처명',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',style:'',id:'column56',value:'오더번호',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column44',value:'컨테이너번호',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column41',value:'B/L번호',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column32',value:'금액',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'90',inputType:'text',id:'column35',value:'진행상태',displayMode:'label'}}]}]},{T:1,N:'w2:gBody',A:{id:'gBody2',style:''},E:[{T:1,N:'w2:row',A:{id:'row4',style:''},E:[{T:1,N:'w2:column',A:{blockSelect:'false',displayMode:'label',id:'selYn',inputType:'checkbox',removeBorderStyle:'false',width:'50',readOnly:'false',trueValue:'1',falseValue:'0',fixColumnWidth:'true',dataType:'number'}},{T:1,N:'w2:column',A:{blockSelect:'false',displayMode:'label',id:'internalTxnBilgNo',inputType:'link',removeBorderStyle:'false',width:'110',class:''}},{T:1,N:'w2:column',A:{blockSelect:'false',displayMode:'label',id:'sysCd',inputType:'select',removeBorderStyle:'false',width:'110',textAlign:'left'}},{T:1,N:'w2:column',A:{blockSelect:'false',displayMode:'label',id:'pchsAcctDeptCd',inputType:'text',removeBorderStyle:'false',width:'120',textAlign:'left',hidden:'true',hiddenCol:'false'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'pchsAcctDeptNm',inputType:'text',width:'160',textAlign:'left'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'bilgAcctDeptCd',inputType:'text',width:'120',textAlign:'left',hidden:'true',hiddenCol:'false'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'bilgAcctDeptNm',inputType:'text',width:'160',textAlign:'left'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'regDtm',inputType:'calendar',width:'100',calendarValueType:'yearMonthDate',displayFormat:'yyyy/MM/dd'}},{T:1,N:'w2:column',A:{displayMode:'label',id:'txnDt',inputType:'calendar',width:'100',calendarValueType:'yearMonthDate',displayFormat:'yyyy/MM/dd'}},{T:1,N:'w2:column',A:{width:'100',inputType:'calendar',id:'realTxnDt',displayMode:'label',calendarValueType:'yearMonthDate',displayFormat:'yyyy/MM/dd'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'bilgClntNo',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',style:'',id:'bilgClntNm',value:'',displayMode:'label',textAlign:'left'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',style:'',id:'odrNo',value:'',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'cntrNo',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'blNo',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'rsltsAmt',displayMode:'label',textAlign:'right',displayFormat:'#,###',dataType:'number',excelCellType:'number',excelFormat:'#,##0'}},{T:1,N:'w2:column',A:{width:'90',inputType:'select',id:'internalTxnStsCd',displayMode:'label'}}]}]},{T:1,N:'w2:footer',A:{style:'',id:'footer1'},E:[{T:1,N:'w2:row',A:{style:'',id:'row5'},E:[{T:1,N:'w2:column',A:{width:'50',inputType:'text',id:'column31',displayMode:'label',fixColumnWidth:'true'}},{T:1,N:'w2:column',A:{width:'110',inputType:'text',id:'column30',displayMode:'label',value:'합계'}},{T:1,N:'w2:column',A:{width:'110',inputType:'text',id:'column29',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column28',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'160',inputType:'text',id:'column27',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column26',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'160',inputType:'text',id:'column25',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'100',inputType:'text',id:'column24',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'100',inputType:'text',id:'column23',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'100',inputType:'text',id:'column52',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column49',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'70',inputType:'text',style:'',id:'column55',value:'',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'70',inputType:'text',style:'',id:'column58',value:'',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column46',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'text',id:'column43',displayMode:'label'}},{T:1,N:'w2:column',A:{width:'120',inputType:'expression',id:'rsltsAmt_s1',displayMode:'label',textAlign:'right',expression:'SUM("rsltsAmt")',displayFormat:'#,##0',dataType:'number'}},{T:1,N:'w2:column',A:{width:'90',inputType:'text',id:'column37',displayMode:'label'}}]}]}]},{T:1,N:'xf:group',A:{class:'grid-bottom ',id:'',style:''},E:[{T:1,N:'xf:group',A:{class:'left',id:''},E:[{T:1,N:'xf:group',A:{class:'grid-total',id:''},E:[{T:1,N:'w2:textbox',A:{class:'',id:'',label:'총',style:'',tagname:'span'}},{T:1,N:'w2:textbox',A:{class:'num',dataType:'number',displayFormat:'#,##0',id:'totalRows',label:'0',style:'',tagname:'span'}},{T:1,N:'w2:textbox',A:{class:'',id:'',label:'건',style:'',tagname:'span'}}]}]}]}]},{T:1,N:'xf:group',A:{class:'btnbox',id:''},E:[{T:1,N:'xf:group',A:{class:'right',id:''},E:[{T:1,N:'w2:udc_comChkRes',A:{style:'width:%; height:px; ;visibility:hidden;',id:'udc_cfJobCloseYN'}},{T:1,N:'xf:trigger',A:{class:'btn white',id:'btn_detail',label:'상세',style:'',type:'button',userAuth:'R','ev:onclick':'scwin.btn_detail_onclick'},E:[{T:1,N:'xf:label',E:[{T:3,text:'상세'}]}]},{T:1,N:'xf:trigger',A:{class:'btn',id:'btn_cancelPchs',label:'매입거부',style:'',type:'button',userAuth:'U','ev:onclick':'scwin.f_CancelPchs'},E:[{T:1,N:'xf:label',E:[{T:3,text:'매입거부'}]}]},{T:1,N:'xf:trigger',A:{class:'btn',id:'btn_cancelPchsReject',label:'매입거부취소',style:'',type:'button',userAuth:'U','ev:onclick':'scwin.f_CancelPchsReject'},E:[{T:1,N:'xf:label',E:[{T:3,text:'매입거부취소'}]}]},{T:1,N:'xf:trigger',A:{class:'btn',id:'btn_pchs',label:'매입처리',style:'',type:'button',userAuth:'U','ev:onclick':'scwin.f_Pchs'},E:[{T:1,N:'xf:label',E:[{T:3,text:'매입처리'}]}]},{T:1,N:'xf:trigger',A:{class:'btn',id:'btn_bill',label:'매입취소',style:'',type:'button',userAuth:'U','ev:onclick':'scwin.f_Bill'},E:[{T:1,N:'xf:label',E:[{T:3,text:'매입취소'}]}]}]}]}]}]}]}]}]})