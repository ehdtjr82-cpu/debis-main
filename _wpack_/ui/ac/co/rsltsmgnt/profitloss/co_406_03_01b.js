...
scwin.f_GridPop = async function (colId, row, data, close) {
  ...
  if (colId == "distAcctDeptCd") {
    ...
    let param = "" + fromDate + "01" + ",,,,givedept";
...
scwin.f_PopUpAutoCreation = function () {
  // var param = ica_fromDate.getValue().trim();
  // var param1 = ds_giveDept.getCellData(ds_giveDept.getRowPosition(), "distAcctDeptCd");
  // var param2 = ds_giveDept.getCellData(ds_giveDept.getRowPosition(), "distAcctDeptNm");
  // var sFeatures = "dialogWidth:450px; dialogHeight:550px;"
  //     + "center:Yes; help:No; resizable:No; status:No; scroll:No";
  // sURL = "/ac/co/rsltsmgnt/profitloss/co_406_03_01p.jsp?stdYm=" + param + "&distAcctDeptCd=" + param1 + "&distAcctDeptNm=" + param2;
  ...
  opt.title = "배부기준등록 자동생성(손익보고)";
  opt.popupName = "배부기준등록 자동생성(손익보고)";
  ...
  //실제호출
  $c.win.openPopup($p, "/ui/ac/co/rsltsmgnt/profitloss/co_406_03_01p.xml", opt, data);
};
...
scwin.f_DistBatch = async function () {
  if (ds_giveDept.getRowCount() <= 0) {
    ...
    await $c.gus.cfAlertMsg($p, MSG_CM_ERR_052, ["배부부서"]);
    return false;
  }
  if ($c.gus.cfIsNull($p, lc_distStdCd.getValue())) {
    // MSG_CM_WRN_007   =  "%1을(를) 선택하십시오."
    await $c.gus.cfAlertMsg($p, MSG_CM_WRN_007, ["배부기준"]);
    return false;
  }
  if (await $c.win.confirm($p, MSG_CM_CRM_016)) {
    ...
    ds_distStdCd.setCellData(row, "distAcctDeptCd", ds_giveDept.getCellData(ds_giveDept.getRowPosition(), "distAcctDeptCd"));
    ds_distStdCd.setCellData(row, "distDegCd", ds_giveDept.getCellData(ds_giveDept.getRowPosition(), "distDegCd"));
    ds_distStdCd.setCellData(row, "distStdCd", lc_distStdCd.getValue());
    $c.sbm.execute($p, sbm_batchDistSts);
  }
};
scwin.sbm_batchDistSts_submitdone = function (e) {
  if (e.responseJSON.resultDataSet[0].Code < 0) return;
  if (e.responseJSON.resultDataSet[0].Msg != "FAIL") {
