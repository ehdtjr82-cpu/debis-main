...
scwin.f_Set = function (gubun) {
  ...
  switch (gubun) {
    case "INIT":
      ...
      $c.gus.cfEnableObjects($p, [cnd_ed_fromdt, cnd_ed_todt, cnd_ed_deptCd, cnd_txt_deptNm]);
    ...
    case "CREATE":
      ...
      // 그리드 수정 가능
      gr_MasterblList.setReadOnly("grid", false);
...
scwin.f_Retrieve = async function () {
  ...
  if (cnd_ed_fromdt.getValue().trim() == '') {
    await $c.gus.cfAlertMsg($p, E_MSG_CM_ERR_002, ["ETD From"]); //@는 필수입력항목입니다.
    cnd_ed_fromdt.focus();
    return;
  }
  if (cnd_ed_todt.getValue().trim() == '') {
    await $c.gus.cfAlertMsg($p, E_MSG_CM_ERR_002, ["ETD To"]); //@는 필수입력항목입니다.
    cnd_ed_todt.focus();
    return;
  }
...
scwin.f_OpenCommonPopUp = function (gubun1, pWinCloseTF, pAllSearchTF, pNoDataCloseTF) {
  switch (gubun1) {
    case "DEPT":
      rtnList = udc_opDeptCdInfo.ilCommonPopUp(scwin.udc_opDeptCdInfo_callBackFunc, cnd_ed_deptCd.getValue().trim(), cnd_txt_deptNm.getValue(), pWinCloseTF, '2', "Code, Name", '80,230', "3,4,5,6,7,8,9,10", null, null, null, null, null, null, pAllSearchTF, "DEPT,Code,Name", pNoDataCloseTF);
...
scwin.f_initSetTab = function () {
  $c.gus.cfEnableObjects($p, [sbm_tab_center0, sbm_tab_center1]);
  cfViewTab(0);
};
...
scwin.f_SendEDI = async function () {
  if (await scwin.f_ReqFieldChk()) {
    ...
    if (ds_BLcheck.getCellData(0, "flag") == -1) {
      await $c.gus.cfAlertMsg($p, E_MSG_CM_ERR_055, ["B/L Error"]); //@ 확인 바람
      ...
      let liner = ds_SeaCargoManifest.getCellData(ds_SeaCargoManifest.getRowPosition(), "liner");
      if (cnd_rd_iogb.getValue() == "I") {
        sbm_ds_customerInfo.action = "/cm.zz.RetrieveComboCMD.do?sysCd=ilCommonEBC&queryId=retriveHouseBlCrn&param1=" + mrnno + "&param2=" + liner;
