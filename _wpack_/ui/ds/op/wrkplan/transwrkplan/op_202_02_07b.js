...
scwin.onpageload = async function () {
  ...
  scwin.isColorChange = false;
  $c.util.clearInterval($p, scwin.colorChangeInterval);
  const codeOptions = [{
    grpCd: "OP190",
...
scwin.onUdcCompleted = function () {
  ...
  $c.gus.cfDisableObjects($p, [btn_print1, btn_print2]);
  ed_em_wrkStDt.focus();
};
scwin.colorChange = async function () {
  if (scwin.isColorChange) {
    if (ds_results.getRowCount() > 0) {
      for (var i = 0; i < ds_results.getRowCount(); i++) {
        if (scwin.count % 2 == 0) {
          if (acb_lc_completeYn.getValue() == "0" || acb_lc_completeYn.getValue() == "") {
            if (ds_results.getCellAllData(i, "twinkleYn") != "0") {
...
scwin.f_switchObjMode = function (gubun) {
  switch (gubun) {
    ...
    case 5:
      ...
      $c.gus.cfEnableObjects($p, [btn_print1, btn_print2]);
      break;
    case 6:
      // 수정불가
      break;
...
scwin.f_emclntExcel = async function (gubun) {
  ...
  if (gubun == null || gubun == "" || gubun == 0) {
    ...
    // udc_topGrdResults2.setGridId("gr_excelResults"); // 개인화버튼에 초기 그리드id 넘겨주기
    // await $c.data.downloadGridViewExcel(gr_excelResults, { fileName: fileName + ".xlsx", sheetName: fileName, removeColumns: "17,18,21,22"});
...
scwin.sbm_retrieve_submitdone = async function (e) {
  if (e.responseJSON.resultDataSet[0].Code == 0) {
    // Grid 정렬 옵션 (dtpDistrictNm, odrKndNm)
    var json = ds_results.getAllJSON();
    json.sort(function (a, b) {
      var d1 = a.dtpDistrictNm || "";
      var d2 = b.dtpDistrictNm || "";
      if (d1 < d2) return -1;
      if (d1 > d2) return 1;
      var o1 = a.odrKndNm || "";
      var o2 = b.odrKndNm || "";
    ...
    // Grid 정렬 옵션
    // var options = {};
    // options.sortIndex = "dtpDistrictNm odrKndNm";
    // options.sortOrder = "1 1"; // 오름차순 : 1, 내림차순 : -1
    ...
    gr_results2.setColumnStyle("xrayLupTrgtYn", "color", "red");
    ...
    // scwin.colorChangeInterval = setInterval(async function() { await scwin.colorChange(); }, 1000);
...
scwin.btn_save_onclick = async function (e) {
  ...
  for (var i = 0; i < ds_results.getRowCount(); i++) {
    if (ds_results.getRowStatus(i) == "U" && ds_results.getRowPosition() != i) {
  ...
  for (var i = 0; i < ds_results2.getRowCount(); i++) {
    ...
    ds_save.setCellData(i, "cntrSeq", Number(ds_results2.getCellData(i, "cntrSeq")));
    ...
    ds_save.setCellData(i, "shtlYn", Number(ds_results.getCellData(ds_results.getRowPosition(), "picConfirmYn")));
...
scwin.btn_fieldClear_onclick = function (e) {
  ...
  acb_lc_lobranCd.setSelectedIndex(7);
...
scwin.f_orderDetail = async function () {
  ...
  if (ds_results.getRowCount() > 0) {
    ...
    if (v_transCargoClsCd == DsConstants.ODR_BULK) {
      ...
      menuId = "sd_402_01_02t.jsp";
      $c.win.openMenu($p, "벌크오더", strOrderDetailUrl + condition, menuId, paramObj);
    } else {
      let params = {
        odrNo: v_odrNo
      };
      ...
      menuId = "sd_402_01_11t.jsp";
      $c.win.openMenu($p, "컨테이너오더", strOrderDetailUrl + condition, menuId, paramObj);
    }
  } else {
...
scwin.formatDateTime = function (value, formatval, rowIdx, colIdx) {
  ...
  if (hhmm && hhmm.length >= 4) {
    halfPart1 = hhmm.substring(0, 2);
    halfPart2 = hhmm.substring(2, 4);
    timePart = halfPart1 + ":" + halfPart2;
  } else if (hhmm && hhmm.length === 2) {
    timePart = hhmm;
  } else {
    timePart = "00";
  }
