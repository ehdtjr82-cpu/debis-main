...
scwin.sbm_block_submitdone = async function (e) {
  if (scwin.wrkPlCd != "" && typeof scwin.wrkPlCd != "undefined") {
    scwin.wrkPlCd = "";
...
scwin.f_retrieve = async function (gubun) {
  ...
  if (gubun == "1") {
    if ($c.util.isEmpty($p, txt_vehclNo.getValue()) && $c.util.isEmpty($p, txt_cntrNo.getValue()) && $c.util.isEmpty($p, txt_odrNo.getValue())) {
      //$c.gus.cfAlertMsg("검색조건중 차량번호나 컨테이너 또는 오더 번호를 입력하세요.");
      //return;
...
scwin.udc_CS_callBackFunc = function (rtnList) {
  if (rtnList != null && rtnList != "N/A") {
    if (rtnList[0] != "N/A") {
      ...
      ds_chassisNoChangeInfo.removeAll();
      ds_chassisNoChangeInfo.insertRow();
      ds_chassisNoChangeInfo.setCellData(0, 'eqCd', ds_outGrid.getCellData(ds_outGrid.getRowPosition(), 'eqCd'));
      ds_chassisNoChangeInfo.setCellData(0, 'logisLnkEqCd', ds_outGrid.getCellData(ds_outGrid.getRowPosition(), 'chassisEqCd'));
...
scwin.f_eqCdSearch = function (bCloseFlag) {
  ...
  // var pURL = '/ds/op/wrkrslts/cywrkrslts/op_301_01_05p3.jsp?paramTitle=가용차량조회';
  // rtnList = window.showModalDialog(pURL, arrParam, "dialogWidth:820px; dialogHeight:530px");
...
scwin.udc_dptArvWrkPlCd_onblurCodeEvent = function (e) {
  //    scwin.udc_dptArvWrkPlCd_openPopup('T');
...
scwin.f_vehclSearch_callBackFunc = function (rtnList) {
  if (rtnList == null) {
    ...
    for (var i = 0; i < rtnList.length; i++) {
      ...
      scwin.hid_posnClsCd = firstList[16];
      scwin.pre_vehclNo = ds_outGrid.getCellData(ds_outGrid.getRowPosition(), 'vehclNo');
    }
  }
};
...
scwin.f_chassisSearch = function (bCloseFlag) {
  ...
  //공통코드 retrieveChassis2
  //    $c.win.alert("공통코드 retrieveChassis2 미개발");
...
scwin.f_chassisSearch_callBackFunc = function (rtnList) {
  ...
  if (rtnList != null) {
    ...
    ds_chassisNoChangeInfo.removeAll();
    ds_chassisNoChangeResult.setData([]);
    // //        dma_chassisNoChangeInfo.reform();
    ds_chassisNoChangeInfo.insertRow();
    ds_chassisNoChangeInfo.setCellData(0, "eqCd", ds_outGrid.getCellData(ds_outGrid.getRowPosition(), "eqCd"));
    ds_chassisNoChangeInfo.setCellData(0, "logisLnkEqCd", ds_outGrid.getCellData(ds_outGrid.getRowPosition(), "chassisEqCd"));
    // dma_chassisNoChangeInfo.set("eqCd", ds_outGrid.getCellData(ds_outGrid.getRowPosition(), "eqCd"));
...
scwin.sbm_chassisNoChangeInfo_submitdone = async function (e) {
  // alert('1');
  // if (e.responseJSON.resultDataSet[0].value == 0) {
...
scwin.sbm_cntrNoChangeInfo_submitdone = async function (e) {
  console.log("### sbm_cntrNoChangeInfo_submitdone : " + JSON.stringify(e));
      ...
      if (ds_cntrNoChangeInfo.getCellData(0, 'cntrNo') == "") {
        ...
        if (ds_outGrid.getCellData(scwin.pre_RowPosition, 'crryinoutClsCd') == DsConstants.CRRYINOUT_CLS_CD_IN) {
          ...
          if (confirm) {
            ...
            //                        gr_gridView.setColumnVisible("cntrNo", false);
            ds_outGrid.setCellData(scwin.pre_RowPosition, 'cntrNo', scwin.pre_cntrNo);
            gr_gridView.setFocusedCell(scwin.pre_RowPosition, 'cntrNo', true);
            //                        gr_gridView.setFocusedCell(0, 0);
            //gr_gridView.setCellReadOnly(rowIndex,"cntrNo", "Edit") = "Upper";	        	
          }
...
scwin.f_gateinProcess = async function (p_gubun) {
  ...
  for (var i = 0; i < ds_outGrid.getRowCount(); i++) {
    if (ds_outGrid.getCellData(i, 'gridChkBox') == "1") {
      //            var sRow = ds_outGridSave.insertRow();
      //            ds_outGridSave.setRowJSON(sRow, ds_outGrid.getRowJSON(i), true);
      //            ds_outGridSave.modifyRowStatus(sRow, "U");
      gridChkYn++;
    }
    var iRow = i + 1;
    ...
    /*
            if (p_gubun == "Auto"
                && ds_outGrid.getCellData(i, 'preInfoSndYn') == "1"
                && ds_outGrid.getCellData(i, 'dptArvWrkPlClsCd') != DsConstants.WRK_PL_CLS_CD_CY
    ...
    */
    ...
    if (p_gubun == "Manual" && ds_outGrid.getCellData(i, 'dptArvWrkPlClsCd') == "6M14" && ds_outGrid.getCellData(i, 'selfClsCd') != DsConstants.SELF_CLS_CD_SELF_CRRYOUT && ds_outGrid.getCellData(i, 'gridChkBox') == "1") {
      ...
      //ds_preInfoSndYn.DataID = "/cm.zz.RetrieveComboCMD.do?sysCd=StationEachRailroadResultsEBC&queryId=RetrievePreInfoSndYnForJSP&param1=" + arvWrkPlCd;
      //ds_preInfoSndYn.Reset();

      dma_preInfoSndYn.set("sysCd", "NcallStationEachRailroadResultsEBC");
  ...
  //		if (!cfValidateGrid2(gr_gridView)) return;
  ...
  if (ds_outGrid.getModifiedIndex().length == 0) {
    ...
    if (confirm) {
      //tr_GateIn.Action = "/ds.op.wrkrslts.cywrkrslts.ProcessGateinCMD.do";
      //tr_GateIn.post();
...
scwin.gr_gridView_oncellclick = function (rowIndex, columnIndex, columnId) {
  ...
  if (colid == "lblock") {} else if (colid == "lbay") {
    ...
    dma_inBay.set("sysCd", "NcallWorkPlaceEachGoodsEBC");
    ...
    //        ds_outRow.DataID = "/cm.zz.RetrieveComboCMD.do?sysCd=WorkPlaceEachGoodsEBC&queryId=retrivecRowList&param1=" + v_wrkPlCd + "&param2=" + v_block + "&param3=" + v_bay;
    //        ds_outRow.Reset();
    dma_inRow.set("sysCd", "NcallWorkPlaceEachGoodsEBC");
    ...
    //        ds_outTier.DataID = "/cm.zz.RetrieveComboCMD.do?sysCd=WorkPlaceEachGoodsEBC&queryId=retrivecTierList&param1=" + v_wrkPlCd + "&param2=" + v_block + "&param3=" + v_bay + "&param4=" + v_row + "&param5=GATEIN";
    //        ds_outTier.Reset();

    dma_inTier.set("sysCd", "NcallWorkPlaceEachGoodsEBC");
  ...
  if (columnId == "odrNo") {} else if (columnId == "cntrTypCd") {
    ...
    //var url = "/ds/op/wrkrslts/cywrkrslts/op_301_01_05p2.jsp";
    //var sFeatures = "status:false;dialogWidth:300px;dialogHeight:200px";

    //showModalDialog(url, ds_outGrid, sFeatures);
    ...
    if (ds_outGrid.getCellData(rowIndex, 'gridChkBox') == "1") {
      ...
      if (ds_outGrid.getCellData(rowIndex, 'lblock') != "") {
        //if (ds_outGrid.getCellData(rowIndex,'lbay') == "") {$c.gus.cfAlertMsg("BAY를 확인하십시요."); ds_outGrid.getCellData(rowIndex,'gridChkBox') = 0;		return;}
        //if (ds_outGrid.getCellData(rowIndex,'lrow') == "") {$c.gus.cfAlertMsg("ROW를 확인하십시요."); ds_outGrid.getCellData(rowIndex,'gridChkBox') = 0;		return;}
        //if (ds_outGrid.getCellData(rowIndex,'ltier') == "") {$c.gus.cfAlertMsg("TIER를 확인하십시요."); ds_outGrid.getCellData(rowIndex,'gridChkBox') = 0;	return;}
      }
      ...
      /*
      if (ds_outGrid.getCellData(ds_outGrid.getRowPosition(), 'offRtrnClsCd') == "1") {
          $c.gus.cfAlertMsg("OFF-반납입니다. 라인담당자에게  문의하세요!!! ");
          ds_outGrid.setCellData(rowIndex, 'gridChkBox', "0");
          return;
      }
      */
      ...
      /*	냉동 체크 임시 주석
      if (ds_outGrid.getCellData(rowIndex, 'cntrTypCd') == DsConstants.CNTR_TYP_CD_RF 
        && ds_outGrid.getCellData(rowIndex,'dryYn') == "0" 
        && ds_outGrid.getCellData(rowIndex, 'temper') == "" 
        && ds_outGrid.getCellData(rowIndex, 'fullEmptyClsCd') == "F") {
          $c.gus.cfAlertMsg("냉동컨테이너 입니다. 온도를 입력하십시요.");
          ds_outGrid.setCellData(rowIndex, 'gridChkBox', "0");
          return;
      }
      if (ds_outGrid.getCellData(rowIndex, 'cntrTypCd') == DsConstants.CNTR_TYP_CD_RF 
        && ds_outGrid.getCellData(rowIndex, 'dryYn') == "1" 
        && ds_outGrid.getCellData(rowIndex, 'temper') != "") {
          $c.gus.cfAlertMsg("냉동컨테이너 DRY 입니다. 온도가 입력 되어 있습니다.");
          ds_outGrid.setCellData(rowIndex, 'gridChkBox', "0");
          return;
      }
      */
...
scwin.gr_gridView_onafteredit = async function (rowIndex, columnIndex, value) {
  ...
  if (colId == 'cntrNo') {
    ...
    if (scwin.pre_vehclNo != ds_outGrid.getCellData(rowIndex, 'vehclNo')) {
      scwin.f_openPopUp('5', '', ds_outGrid.getCellData(rowIndex, 'vehclNo'), 'T', 'T');
      //            scwin.f_vehclSearch('T');
    }
    ...
    */
    if (ds_outGrid.getCellData(rowIndex, 'gateinDth').substr(4, 2) < 1 || ds_outGrid.getCellData(rowIndex, 'gateinDth').substr(4, 2) > 12) {
    ...
    if ($c.date.getDateQuarterDirection($p, ds_outGrid.getCellData(rowIndex, 'gateinDth').substr(0, 4), ds_outGrid.getCellData(rowIndex, 'gateinDth').substr(4, 2), "L").substr(6, 2) < ds_outGrid.getCellData(rowIndex, 'gateinDth').substr(6, 2) || ds_outGrid.getCellData(rowIndex, 'gateinDth').substr(6, 2) < "1") {
...
scwin.ds_outGrid_oncelldatachange = function (info) {
  ...
  if (colID == "ltier") {
    ...
    // ds_CntrPositionInfo.DataId = "/ds.op.wrkrslts.cywrkrslts.RetrieveShippingWeightPositionContentsCMD.do"
    // 				+ "?wrkPlCd="+ds_Grid1.NameValue(row,'wrkPlCd')
    // 				+ "&lblock="+ds_Grid1.NameValue(row,'lblock')
    // 				+ "&lbay="+ds_Grid1.NameValue(row,'lbay')
    // 				+ "&lrow="+ds_Grid1.NameValue(row,'lrow')
    // 				+ "&ltier="+ds_Grid1.NameValue(row,'ltier')
    // 				;
    // ds_CntrPositionInfo.Reset();
