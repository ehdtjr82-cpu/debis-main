...
scwin.lastLoginAttemptUserId = "";
scwin.lastLoginAttemptPasswd = "";
scwin.onpageload = function () {
  scwin.failedPasswordCount = 0;
  scwin.clearPendingLoginPopup();
  $c.sbm.execute($p, sbm_logout);
  const serverMode = window.location.hostname.indexOf("newdevdebis.dongwon.com") > -1 ? "dev" : window.location.hostname.indexOf("newtest.dongwon.com") > -1 ? "test" : window.location.hostname.indexOf("newdebis.dongwon.com") > -1 ? "prd" : "local";
  if (serverMode === "local") {
...
scwin.captureLoginAttempt = function (userId, passwd) {
  scwin.lastLoginAttemptUserId = (userId || "").trim();
  scwin.lastLoginAttemptPasswd = passwd || "";
};
...
scwin.isPasswordExpiryWarningMessage = function (message) {
  return scwin.hasErrorCode(message, ["E0039"]);
};
scwin.isPasswordResetMessage = function (message) {
  return scwin.hasErrorCode(message, ["E0025"]);
};
...
scwin.openLegacyPopup = function (type) {
  ...
  const popupUserId = scwin.lastLoginAttemptUserId || (txt_id.getValue() || "").trim();
  const popupPasswd = scwin.lastLoginAttemptPasswd || txt_pwd.getValue() || "";
  let popupData = null;
  const options = {
  ...
  if (type === "3") {
    return $p.openPopup(popupUrl, options);
  }
  if (popupData) {
    return $c.win.openPopup($p, popupUrl, options, popupData);
  }
...
scwin.getLoginErrorAlertOptions = function (message) {
  ...
  if (scwin.isPasswordExpiryWarningMessage(message)) {
    return {
      closeCallbackFncName: scwin.onLoginErrorAlertClosed,
      callBackParam: {
        popupType: "4"
...
scwin.handlePostLoginChecks = async function (userInfo) {
  ...
  if (!userMpNo && !userMpNo2) {
    await scwin.openUserMpNoPopup(userInfo);
...
scwin.handleLoginFailure = async function (message) {
  ...
  if (scwin.isPasswordChangeRequiredMessage(message)) {
    scwin.captureLoginAttempt(txt_id.getValue(), txt_pwd.getValue());
  ...
  if (scwin.isPasswordExpiryWarningMessage(message)) {
    scwin.captureLoginAttempt(txt_id.getValue(), txt_pwd.getValue());
    scwin.scheduleLegacyPopupAfterCommonAlert("4");
    scwin.resetFailureCount();
    txt_pwd.setValue("");
    txt_id.focus();
    return;
...
scwin.btn_trigger1_onclick = async function () {
  ...
  try {
    ...
    scwin.captureLoginAttempt(userId, passwd);
...
}}}]}]},{T:1,N:'body',A:{'ev:onpageload':'scwin.onpageload','ev:onbeforepageunload':'scwin.onbeforepageunload'},E:[{T:1,N:'xf:group',A:{class:'pop_contents login-pop',id:''},E:[{T:1,N:'w2:textbox',A:{class:'logo_loex',id:'',label:'동원로엑스'}},{T:1,N:'xf:group',A:{class:'login-inner',id:''},E:[{T:1,N:'w2:textbox',A:{class:'login-title',id:'',label:'로그인'}},{T:1,N:'xf:input',A:{id:'txt_id',placeholder:'아이디를 입력해 주세요.','ev:onkeydown':'scwin.onkeydown'}},{T:1,N:'xf:group',A:{class:'input_pw',id:''},E:[{T:1,N:'w2:searchbox',A:{id:'txt_pwd',placeholder:'비밀번호를 입력해 주세요.',type:'password','ev:onkeydown':'scwin.onkeydown'}}]},{T:1,N:'xf:trigger',A:{id:'btn_trigger1',title:'로그인',type:'button','ev:onclick':'scwin.btn_trigger1_onclick','ev:onkeydown':'scwin.onkeydown'},E:[{T:1,N:'xf:label',E:[{T:4,cdata:'로그인'}]}]},{T:1,N:'xf:group',A:{class:'etc-box',id:''},E:[{T:1,N:'xf:group',A:{class:'form-group',id:''},E:[{T:1,N:'w2:textbox',A:{id:'',label:'CapsLock :',tagname:'span'}},{T:1,N:'w2:textbox',A:{id:'tbx_capslock',label:'',tagname:'span'}}]},{T:1,N:'xf:select',A:{appearance:'full',id:'chk_rememberId',renderType:'checkboxgroup',selectedindex:'-1',cols:'1'},E:[{T:1,N:'xf:choices',E:[{T:1,N:'xf:item',E:[{T:1,N:'xf:label',E:[{T:4,cdata:'아이디 저장'}]},{T:1,N:'xf:value',E:[{T:4,cdata:'Y'}]}]}]}]}]}]}]}]}]}]})