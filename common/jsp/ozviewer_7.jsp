...
<html> 
 <head> 
  <script language="javascript" >
    ...
    function jsOzJobPrintEnd( mg, cd) { 
	 try { 	 
        parent.cfOzJobPrintEnd( mg, cd);
     } catch ( Exception) { 
      // ignore 
     } 
	}	
...
<body>
<% } else { %>
<body onLoad="txt_dummy.focus();jsOzJobBegin();"> 
<input type='text' id='txt_dummy'>
<% } %>
<comment id='__NSID__'><OBJECT width = "0" height = "0" ID="ZTransferX" CLASSID="<%=DOZConstants.ZTransferX_CLSID%>" 
	codebase="<%=DOZConstants.ZTransferX_CODEBASE%>">
	<PARAM NAME="download.Server" VALUE="<%=DOZConstants.ZTransferX_DOWNLOAD_SERVER%>">
	...
	<PARAM NAME="usewinhttp" VALUE="true">
...
<%
   ...
   HOST_NAME_ONLY ="devdebis.dongwon.com";
//    String OZ_SERVLET = "http://"+ HOST_NAME_ONLY + "/oz70" + DOZConstants.CONNECTION_SERVLET ;
   String OZ_SERVLET = "http://devdebis.dongwon.com/oz70/server/" ;
   System.out.println("-----------------------------------------" + OZ_SERVLET);
  
%>
<comment id='__NSID__'><OBJECT id="<%=viewername%>" CLASSID="<%=DOZConstants.VIEWER_CLSID%>" width="100%" height="100%">
   <!-- 
   ...
   -->
   <param name="connection.servlet" value="<%=OZ_SERVLET%>">
