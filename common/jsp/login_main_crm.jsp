...
<%!/** 
	 * @source  : $Source: /CVS/cvs/cvsrepo/DBL_PROJECT/DBL_EAR/WEB_ROOT/common/jsp/login_main_crm.jsp,v $
	 * @date    : $Date: 2016/05/10 12:30:19 $
...
<script language="javascript" src="/ds/common/script/dscommon.js"></script>
...
String sabun	=	request.getParameter("sabun") ==null? "" : request.getParameter("sabun") ; //포르테에서 넘어온값
			...
			if (LoginUtil.getLoginDTO(request) != null
					&& !"csssso".equals(login.getUserId())) {
			%>
<script language="javascript"> 
location.href = "/main_frame_crm.html";
</script>
<%} else {
...
<script language="javascript">    
  ...
  function f_HeaderCreate(){
	...
	var GAUCE_DATASET_HEADER = "userId:STRING(8),passwd:STRING(12),passwdCheckCnt:INT(2),isPostponePasswdChg:INT(1),blockReason:STRING(1),authLoginYn:STRING()";
	oGDS_login.ClearAll();
	...
	var DATASET_SALES_HEADER = "userId:STRING(8), salesChk:DECIMAL(1)";
	sales_login.ClearAll();
	sales_login.SetDataHeader(DATASET_SALES_HEADER);
	sales_login.AddRow();
  } 
  ...
  function jsSSOLogin() {   
	...
	if(EXIST_YN==1) {
		...
		var SSO_GAUCE_DATASET_HEADER = "empNo:STRING(255),empNm:STRING(255),email:STRING(255),authLoginYn:STRING()";
...
<object id="tr_salesLogin" classid="<%=DGauceCLSID.TR%>">
  <param name="KeyName"  value="toinb_dataid4">
  <param name="KeyValue" value="Servlet(I:IN_GDS1=sales_login, O:OUT_GDS1=sales_loginResult)">
</object>


<!-- 영업정보시스템 접근권한 유무 체크를 위한  DataSet 선언 -->
<object id="sales_login" classid="<%=DGauceCLSID.DATASET%>"> </object>

<!-- 영업정보시스템 접근권한 유무 결과를 받기 위한  DataSet 선언 -->
<object id="sales_loginResult" classid="<%=DGauceCLSID.DATASET%>"> 
...
<script language="JavaScript" for="tr_login" event="OnSuccess()">
    ...
    //alert("영업정보");
    sales_login.NameValue(1,"userId") = document.loginForm.txt_userId.value;     
    //dataSetDebug(sales_login, true);
    
    //tr_salesLogin.UseChangeInfo = false ; 
    tr_salesLogin.Action = "/cm.bc.salesmgnt.RetrieveSalesLogin2CMD.do";     
    tr_salesLogin.Post();
    //dataSetDebug(sales_loginResult, true);
     
 	if(sales_loginResult.NameValue(1,"salesChk")== 1){
 			location.href = "/main_frame_crm.html";   //로그인
 	}else{ 
 			oGDS_logout_result.DataID = "/logout.do"; //로그아웃
        	oGDS_logout_result.Reset(); 
 			return;	//로그인 안됨
 	}
...
<script language="JavaScript" for=tr_salesLogin event="OnSuccess()">

    //0이면 권한없음, 1이면 권한있음        
    if(sales_loginResult.NameValue(1,"salesChk")==0){
     alert("영업정보시스템 사용자가 아닙니다. 사용신청 후 사용해주십시오.");
     return;
    }

</script>

<script language="JavaScript" for="tr_salesLogin" event="OnFail()">  
   cfShowError(tr_salesLogin);
</script>
		...
		<table width="707" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td height="33" align="left" valign="top"><a href="#"><img
					src="/common/images/ci.gif" width="147" height="24" border="0"></a></td>
			</tr>
			<tr>
				<td>
								...
								<table width="240" height="141" border="0" cellpadding="0"
									...
									<tr>
										...
										<td align="left" valign="bottom"><input id="txt_passwd"
											...
											style="width:98px; height:19px" maxlength="8"
											onKeyPress="checkCapsLock( event )"
											onkeydown="checkCapsLock( event );"></td>
									</tr>
									<tr align="center" valign="bottom">
										<td height="11" colspan="3"><img src="/common/images/dot.gif"
				...
				<table width="692" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td height="27" align="left"><img
							src="/common/images/copyright.gif" width="327" height="8"></td>
					</tr>
				</table>
				</td>
...
<object id=nBind classid="<%=DGauceCLSID.BIND%>">
	<param name=DataID value=sales_login>
	<param name="BindInfo"
		value='
	    <C> Col=userId             Ctrl=txt_userId         Param=value   </C>
	'>
