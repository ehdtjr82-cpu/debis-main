...
<%@ page import="dbl.cm.fw.constants.*, dbl.cm.bc.comnmgnt.usermgnt.util.*, dbl.cm.bc.comnmgnt.usermgnt.dto.*" %>

<%
	LoginDTO dto = LoginUtil.getLoginDTO(request);
	String userClsCd = dto.getUserClsCd();
	String coCd = dto.getCoCd();
	String basisMenuClsCd = dto.getBasisMenuClsCd();

	String userType = userClsCd ;
	if ( basisMenuClsCd == null )  basisMenuClsCd = "01";
	
	if ("01".equals(userType)) {
	    userType = dto.getUserHomeClsCd();
	    if("017".equals(coCd)) {//LA해외법인 SA에서 LA로 강제 매핑 변경
	    	userType = "LA";
	    }
...
var clickedSysCd = "<%=userType%>";
var userHomeClsCd = "<%=userType%>";

function f_OnLoad() { 
	cfMenuSelect("<%=userType%>");
	cfChangeMenuWidth(frameObj, tv_menu, MENU_FRAME_DEFAULT_WIDTH);
	tv_menu.Focus();	
	
	<%if("91".equals(userClsCd)){%>
    tv_menu.ExpandLevel = "3";
    <%}%>
}

function cfMenuSelect(sysCd) {
	tv_menu.ClearAll();

    if (sysCd == "admin") {
    	tv_menu.style.display = "none";
    	tv_tmpMenu.style.display = "";
    	
	    ds_tmpMenuList.DataID = "/cm.bc.comnmgnt.menumgnt.cmd.RetrieveAllMenuListCMD.do";
	    ds_tmpMenuList.Reset();
	}
    else {
    	tv_tmpMenu.style.display = "none";
    	tv_menu.style.display = "";
    	clickedSysCd = sysCd;
    	
	    ds_menuList.DataID = "/cm.bc.comnmgnt.menumgnt.cmd.RetrieveMenuListCMD.do?sysCd=" + sysCd+"&userHomeClsCd="+userHomeClsCd;
	    ds_menuList.Reset();
	...
	// My메뉴가 기본일경우
	if ( "<%=basisMenuClsCd%>" == "02" )
	{
		parent.f_SelectMyMenuTab(frameObj,tv_menu);
		cfChangeMenuWidth(frameObj, tv_menu, MENU_FRAME_DEFAULT_WIDTH);	
	}
...
function f_SelectMyMenuTab_Call()
{
	cfChangeMenuWidth(frameObj, tv_menu, MENU_FRAME_MAX_WIDTH); 
	parent.f_SelectMyMenuTab(frameObj,tv_menu);
...
function f_NewScreen_(pUrl)
{
	/*
        ...
        pWin.focus();
	*/
		pWin = window.open("", "dbl","") ;
        pWin.location.href = "/index.html";
...
function f_DelayCall() {
	f_SelectMyMenuTab_Call();
	cfChangeMenuWidth(frameObj, tv_menu, MENU_FRAME_DEFAULT_WIDTH);	
}
...
{
	if(txt_srchMenu.value == ""){
		alert("검색어를 입력하세요");
		return false;
	}
	var searchMenuCond = txt_srchMenu.value;
    ds_menuList.DataID = "/cm.bc.comnmgnt.menumgnt.cmd.RetrieveSearchMenuListCMD.do?sysCd=" + clickedSysCd+"&menuNm="+searchMenuCond;
    ds_menuList.Reset(); 
...
<object id="ds_tmpMenuList" classid="<%=DGauceCLSID.DATASET%>">
	<param name=SyncLoad	value="true">
</object>
...
<script language=JavaScript for=ds_menuList event=OnLoadCompleted(rowcnt)>
	...
	if(txt_srchMenu.value == ""){
		tv_menu.ExpandLevel = "0";
	}else{
		tv_menu.ExpandLevel = "4";
	}
    ...
    //window.showModalDialog(strCertiNoDetailUrl+condition,window,"dialogHeight:600px;dialogWidth:700px;"); 	
...
<script language=JavaScript for=ids_tvImage event=OnLoadCompleted(rowcnt)>
tv_menu.Focus();
//if ("<%=basisMenuClsCd%>" == "02") {
//		setTimeout("f_DelayCall()", 1000);
//}
</script>

<script language=JavaScript for=ds_tmpMenuList event=OnLoadCompleted(rowcnt)>
	tv_menu.Focus();
</script>
<script language="javascript" for=ds_tmpMenuList event=OnLoadError()>
    cfShowError(ds_tmpMenuList);
</script>

<script language="javascript" for=tv_menu event=OnItemClick()>
	var pageUrl = ds_menuList.NameValue(ds_menuList.RowPosition,"actPgmScrPath")
				+ ds_menuList.NameValue(ds_menuList.RowPosition,"actPgmId");
	...
	//var acPgm = ds_menuList.NameValue(ds_menuList.RowPosition,"actPgmId") ;
    //if ( "<%=userClsCd%>" == "05" && acPgm == "op_301_02_11b.jsp" ) {
    //	alert("TEL : 02-508-5553, 02-508-5554, 02-508-5595, 02-508-2225\nFAX : 02-508-5662\n\n반입/반납 예정정보를 FAX번호로 보내주세요..\n빨리 복구될 수 있도로 하겠습니다. 죄송합니다.");
    //	return;
    //}
...
<script language="javascript" for=tv_tmpMenu event=OnItemClick()>
	var pageUrl = ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"actPgmScrPath")
				+ ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"actPgmId");

	if (ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"actPgmParam") != "") {
		pageUrl += "?" + ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"actPgmParam");
		
		if (ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"menuParam") != "")
			pageUrl += "&" + ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"menuParam");
	}
	else {
		if (ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"menuParam") != "")
			pageUrl += "?" + ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"menuParam");
	...
	var menuNm = ds_tmpMenuList.NameValue(ds_tmpMenuList.RowPosition,"menuNm");
...
<table height="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		...
		<table width="180" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td><img src="/common/images/left_tap01_on.gif" name="Image21" width="90" height="34" border="0"></td>
				<td><a href="#" onClick="f_SelectMyMenuTab_Call();" onfocus="this.blur()"><img
					src="/common/images/left_tap02_off.gif" name="Image22" width="90" height="34" border="0"></a></td>
			</tr>
		</table>
		</td>
	</tr>
	...
	<tr height=10 valign=center>
		<td width=1 bgcolor=#E3E3E3>&nbsp;</td>		
		<td align=left>
			<INPUT TYPE="text" class=input_text	id="txt_srchMenu" size="12" onkeypress="if(event.keyCode==13) f_searchMenu();" style = "ime-mode:active"> 
			<input type="button" class="button_newwin" value="찾기" onclick="f_searchMenu();">&nbsp;
		</td>
	...
	<tr height="100%"> 
		...
		<comment id='__NSID__'><object id=tv_tmpMenu classid="<%=DGauceCLSID.TREEVIEW%>" style="height:100%; width:340;">
			<param name="FontSize"   value="10">
			<param name="FontName"   value="돋움">
			<param name=SingleExpand value="true">
			<param name=ExpandLevel  value="0">
			<param name=BorderStyle  value="0">
			<param name=DataID       value="ds_tmpMenuList">
			<param name=TextColumn   value="menuNm">
			<param name=LevelColumn  value="lvl">
			<param name=UseImage     value="false">
			<param name=BackImage    value="/common/images/left_bottom_back.jpg">
