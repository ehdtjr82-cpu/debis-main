...
function cfAddItem<%=DUMMY%>(idx, val) {
	...
	if (containsChars(cfGetFileName<%=DUMMY%>( seq ), "'!,+&^%$#@~;"))
	{
		alert("파일명, 파일경로에 특수문자를 입력하실 수 없습니다.\n(특수문자 종류 : '!,+&^%$#@~;)");
		return ;
	}
...
<div style="width:100%;height:65;overflow:auto" id='<%=DISPLAY_ITEMS%>'></div>
</fieldset>
<input type="hidden" name="INIT_ITEM_LIST_FUNCTION" value="cfInitItemList<%=DUMMY%>"> 
<input type="button" value="찾아보기"  style="cursor:hand;height:25">
<div style='width:100%;height:15;overflow:hidden;position:relative;top:-20px;filter:alpha(opacity=0);-moz-opacity:0'>
