...
function cfAddItem<%=DUMMY%>(idx, val) {
	...
	var display_script = '<span id='+'<%=DISPLAY_ITEM%>'+'_'+idx+'>'+ cfTrimSize<%=DUMMY%>(val) +' <b onclick=cfCancelItem<%=DUMMY%>('+seq+') style=cursor:pointer> &nbsp;&nbsp; <img src="/common/images/button_cancel.gif" style=cursor:pointer  align=absmiddle>  </b> </span><br>'; 
...
<div style="width:100%;height:<%=DISPLAY_HEIGHT%>;overflow:auto" id='<%=DISPLAY_ITEMS%>'></div>
</fieldset>
<input type="hidden" name="INIT_ITEM_LIST_FUNCTION" value="cfInitItemList<%=DUMMY%>"> 
<img src="/common/images/attach.gif" style=cursor:pointer align=absmiddle>	 <font style=font-size:9pt;text-decoration:underline;> 첨부파일 추가 </font>
<div style='width:100%;height:16;overflow:hidden;position:relative;top:-20px;filter:alpha(opacity=0);-moz-opacity:0'>
